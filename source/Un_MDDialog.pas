...
type
  ...
  Protected
    ...
    // procedimento que carraga a permissão do usuario logado para o formulario em questão
    procedure LoadPermisso;     virtual;
    // procedimento que carrega as restrilçies dos campos do formulario em quetão
  ...
  case FRecordState of
    rsNew     :
          if  TConfig.GetInstance.ConfirmaGravacao then
              if MessageDlg('Deseja Cancelar a Inclusão?', mtConfirmation,mbYesNo, 0)= mrNo then begin exit;  end;
    rsEdit    :
          if  TConfig.GetInstance.ConfirmaGravacao then
              if MessageDlg('Deseja Cancelar a Edição', mtConfirmation,mbYesNo, 0)= mrNo then begin exit; end;
    rsDelete  :
          if  TConfig.GetInstance.ConfirmaGravacao then
              if MessageDlg('Deseja Cancelar a Exclusão?', mtConfirmation,mbYesNo, 0)= mrNo then begin exit;  end;
  end;
...
begin
  ...
  case FRecordState of
   ...
   rsDelete  : if MessageDlg('Confirma Exclusão?', mtConfirmation,mbYesNo, 0) = mrYes   then dsCrud.DataSet.Delete  Else  exit;
  end;
  ...
  try
      ...
      // Inserindo informações no TRANSFER
  ...
  Except
    on E : Exception do
      begin
        { Implementando informações de erros}
        case FRecordState of
         rsNew     :
            begin
                cMSG := 'Erro de Inclusão: '+#13+#13
                       +'Todas os dados desta movimentação foram desconsiderados, consulte o suporte!'+#13+#13
                       +E.ClassName+' error raised, with message : '+E.Message;
                MessageDlg(cMSG, mtWarning ,[mbAbort], 0);
            end;
         rsEdit    :
            begin
                cMSG := 'Erro de Alteração: '+#13+#13
                         +'Todas as alterações feitas nesta sessão serão desconsideradas, consulte o suporte!'+#13+#13
                         +E.ClassName+' error raised, with message : '+E.Message;
                MessageDlg(cMSG, mtWarning,[mbAbort], 0);
            end;
         rsDelete :
            begin
                cMSG := 'Erro de Exclusão: '+#13+#13
                        +'Este registro não pode ser excluído devido as regras de integridade de informações, consulte o suporte!'+#13+#13
                         +E.ClassName+' error raised, with message : '+E.Message;
                MessageDlg(cMSG, mtWarning,[mbAbort], 0);
            end;
        end;
...
begin
  ...
  lbTitulo.Caption               :=  'Formulário de '+UpperCase(Copy(cTabela,1,1))+Copy(cTabela,2,length(cTabela))  ;
  ...
  btnVizualiza.Hint := 'Vizualize Registro|Pressione para Ver o Formulário '+#13+' de um registro selecionado,'+#13+' ou precione as teclas CTRL+I '+#13+' no seu teclado '+#13+'|300';
  btnCancela.Hint   := 'Cancelar Processos|Pressione para cancelar '+#13+' processos de Inserção/Edição/Exclusão,'+#13+' ou precione as teclas F12 '+#13+' no seu teclado '+#13+'|300';
  btnGravar.Hint    := 'Grava Processos|Pressione para Executar '+#13+'  processos de Inserção/Edição/Exclusão,'+#13+' ou precione as teclas CTRL+S '+#13+' no seu teclado '+#13+'|300';
  btnInicio.Hint    := 'Navegação |Vai para o inicio '+#13+' do(s) registro(s) da tela,'+#13+' ou precione as teclas HOME'+#13+' no seu teclado '+#13+'|300';
  btnAnterior.Hint  := 'Navegação |Vai para o anterrior '+#13+' registro da tela,'+#13+' ou precione as teclas SETA < '+#13+' no seu teclado '+#13+'|300';
  btnProximo.Hint   := 'Navegação |Vai para o próximo '+#13+' registro da tela,'+#13+' ou precione as teclas SETA >'+#13+' no seu teclado '+#13+'|300';
  btnTermino.Hint   := 'Navegação |Vai para o final '+#13+' do(s) registro(s) da tela,'+#13+' ou precione as teclas END'+#13+' no seu teclado '+#13+'|300';
  btnSair.Hint      := 'Sair |Sai do sistema ou '+#13+' precione as teclas ESC'+#13+' no seu teclado '+#13+'|300';
  edPesquisa.Hint   := 'Pesquisar | Clique no cabeçalho da coluna '+#13+' do campo a ser pesquisado, '+#13+' depois digite sua pesquisa '+#13+'|300';
