...
REM LO_Slos.dll
@SET LO_Slos="C:\\\\ProgramData\\\\Jenkins\\\\.jenkins\\workspace\\\\NovoTeste\\\\source\\\\SGA.dproj"
@ECHO %START_BUILD_STR% %LO_Slos%
DEL "C:\\\\ProgramData\\\\Jenkins\\\\.jenkins\\workspace\\\\NovoTeste\\\\source\\\\SGA.exe"
REM Limpando a compilanção anterior
MSBuild %LO_Slos% -t:clean -p:config=%BUILD_CONFIG% -p:platform=Win32
REM Compilando projeto
MSBuild /property:DCC_UnitSearchPath=FortesReportkDir %LO_Slos% -t:Build -p:config=%BUILD_CONFIG% -p:platform=Win32
